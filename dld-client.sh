#!/bin/bash
#
# Distributed downloading script for Anyhub.
#
# This will get a prefix from the tracker and download data
# from Anyhub.
#
...
do
  # request a prefix
  echo -n "Getting next prefix from tracker..."
  prefix=$( curl -s -f -d "{\"downloader\":\"${youralias}\"}" http://anyhub.heroku.com/request )

  # empty?
  if [ -z $prefix ]
  then
    echo
    echo "No prefix. Sleeping for 30 seconds..."
    echo
    sleep 30
  else
    echo " done."
